...
- 代码实现完成并通过本地质量门禁后，默认直接执行 `git commit` 与 `git push`，无需等待用户再次提醒。
- 若本地质量门禁失败、远端推送被拒绝、涉及 destructive action、历史重写、force push 或无法确认变更范围，应停止并向用户报告原因。
- commit 前必须复查 `git status`、`git diff --check` 与本次变更范围，避免提交临时产物、代理/换源配置或无关文件。
