...
func TestWriteInputsExcludeServerManagedTimestamps(t *testing.T) {
	...
	session := SessionInputFrom(Session{ID: 1, Name: "server", Tags: "database,production", Notes: "critical", Environment: "production", Project: "payments", CreatedAt: now, UpdatedAt: now}).Session()
	...
	assert.Equal(t, "database,production", session.Tags)
	assert.Equal(t, "critical", session.Notes)
	assert.Equal(t, "production", session.Environment)
	assert.Equal(t, "payments", session.Project)
