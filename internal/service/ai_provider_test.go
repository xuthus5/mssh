...
func TestProviderDefaultsAndValidation(t *testing.T) {
	...
	assert.Error(t, validateProviderURL(model.AIProviderProfile{BaseURL: "file:///etc/passwd"}))
	assert.Error(t, validateProviderURL(model.AIProviderProfile{BaseURL: "http://0.0.0.0:8080"}))
