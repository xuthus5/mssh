...
func TestSessionService_ExportCSVIncludesPasswordsAndRejectsInvalidSelection(t *testing.T) {
	...
	service.SetPasswordVerifier(staticPasswordVerifier("initial-pass-12"))
	result, err := service.ExportCSV(path, model.SessionCSVExportOptions{IncludePasswords: true, ConfirmPassword: "initial-pass-12"})
	require.NoError(t, err)
