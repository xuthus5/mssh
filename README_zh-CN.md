...
桌面端提供经过认证的统一本地 IPC 通道用于灰度验证；默认安装包暂时保留 Wails 兼容通信路径，待跨平台稳定性验证完成后再切换。终端输入已按批次发送，避免每个字符触发一次 RPC。
...
- 历史命令预测补全：输入前缀或整条命令时按最近执行排序给出候选，Shift+Tab 采纳，Tab 保留给 Shell
- 终端输出搜索，支持文本与正则模式
- 会话录制回放
...
- SSH 建立连接、指纹确认、连接成功在同一弹框内完成，成功后自动聚焦终端
- 会话可选 SSH 连接隧道（跳板机），支持连接测试和阶段进度，用于访问内网主机
- 总览中管理已信任主机指纹
