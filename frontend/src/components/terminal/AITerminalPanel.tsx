...
export function AITerminalPanel({ terminalID, sessionID, onClose }: { terminalID: string; sessionID: number; onClose: () => void }) {
  ...
  useEffect(() => { void loadPanel().catch(() => undefined) }, [sessionID])
  ...
  return <aside style={panel.panelStyle} className="absolute inset-y-0 right-0 z-20 flex min-w-0 flex-col border-l border-border bg-card shadow-xl" data-testid="ai-terminal-panel"><div {...panel.resizeHandleProps} className="absolute inset-y-0 -left-1 z-30 w-2 cursor-col-resize touch-none outline-none after:absolute after:inset-y-0 after:left-1/2 after:w-px after:-translate-x-1/2 after:bg-transparent hover:after:bg-primary/60 focus-visible:after:bg-primary active:after:bg-primary" /><header className="flex items-center justify-between border-b border-border px-3 py-2"><span className="flex items-center gap-2 text-sm font-semibold"><Bot className="size-4 text-primary" />{t('AI 运维')}</span><span className="flex items-center gap-1"><Button size="icon-xs" variant={historyOpen ? 'secondary' : 'ghost'} aria-label={t('对话历史')} onClick={() => setHistoryOpen((value) => !value)}><History /></Button><Button size="icon-xs" variant="ghost" aria-label={t('关闭 AI 面板')} onClick={onClose}><X /></Button></span></header>{historyOpen && <div className="max-h-48 overflow-y-auto border-b border-border p-2">{error && conversations.length === 0 ? <div className="space-y-2 p-2 text-xs text-destructive" role="alert"><p>{t('加载 AI 面板失败: ${}', error)}</p><Button size="xs" variant="outline" onClick={() => { void loadPanel().catch(() => undefined) }}>{t('重试')}</Button></div> : conversations.length === 0 ? <p className="p-2 text-xs text-muted-foreground">{t('暂无对话')}</p> : conversations.map((conversation) => <button type="button" key={conversation.id} className="block w-full truncate rounded-md px-2 py-1.5 text-left text-xs hover:bg-muted" onClick={() => { void loadConversation(conversation.id).catch(() => undefined) }}>{conversation.title}</button>)}</div>}<div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-3">{messages.length === 0 && <div className="rounded-lg border border-dashed border-border p-4 text-xs text-muted-foreground">{t('从当前终端上下文开始提问。模型不会自动执行命令，命令必须逐条审批。')}</div>}{messages.map((message) => <MessageView key={message.id} message={message} sessionID={sessionID} terminalID={terminalID} conversationID={conversationID} />)}{pending && <p className="text-xs text-muted-foreground">{t('AI 正在分析当前上下文...')}</p>}{error && <p className="rounded-md bg-destructive/10 p-2 text-xs text-destructive">{error}</p>}</div><div className="border-t border-border p-3"><div className="mb-2 flex items-center justify-between text-xs text-muted-foreground"><label className="flex items-center gap-2"><Switch checked={useSearch} onCheckedChange={setUseSearch} />{t('网络搜索')}</label><span>{contextLines} {t('行上下文')}</span></div><div className="flex items-end gap-2"><Textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && (event.metaKey || event.ctrlKey)) void send().catch(() => undefined) }} placeholder={t('描述要排查或执行的运维任务')} rows={3} /><Button size="icon" disabled={!canSend} aria-label={t('发送问题')} onClick={() => { void send().catch(() => undefined) }}><Send /></Button></div></div></aside>
}
...
function CommandProposal({ command, sessionID, terminalID, conversationID, initiallyExecuted }: { command: AICommandProposal; sessionID: number; terminalID: string; conversationID: number; initiallyExecuted: boolean }) {
  ...
  const execute = async () => { try { await AIService.ExecuteCommand({ conversation_id: conversationID, session_id: sessionID, terminal_id: terminalID, command: command.command, approved: true }); setExecuted(true); setError('') } catch (executionError) { const message = errorMessage(executionError); setError(message) } }
  return <Card className="border-border bg-background"><CardContent className="space-y-2 p-2.5"><div className="flex items-start gap-2"><code className="min-w-0 flex-1 whitespace-pre-wrap break-all text-xs">{command.command}</code>{blocked ? <ShieldAlert className="size-4 shrink-0 text-destructive" /> : executed ? <Check className="size-4 shrink-0 text-emerald-600" /> : <Button size="xs" disabled={blocked} onClick={() => { void execute().catch(() => undefined) }}>{t('审批并执行')}</Button>}</div><p className="text-xs text-muted-foreground">{command.purpose || t('模型建议命令')} · {riskLabel(command.risk)}</p>{error && <p className="text-xs text-destructive">{error}</p>}{blocked && <p className="text-xs text-destructive">{command.blocked_reason || t('命令被安全策略阻断')}</p>}</CardContent></Card>
}
