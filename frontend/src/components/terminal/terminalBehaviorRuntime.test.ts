...
describe('terminal behavior runtime', () => {
  ...
  it('follows copy-on-select settings and disposes only once', () => {
    ...
    useTerminalBehaviorStore.getState().setSettings({ rightClickAction: 'paste', copyOnSelect: true, scrollbackLines: 10000, autoReconnect: false, restoreTabsOnStartup: true, renderer: 'dom', historyPredict: false, autoCloseTerminalOnExit: false })
    cleanup()
    cleanup()
