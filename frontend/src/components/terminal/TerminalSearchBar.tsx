...
export function TerminalSearchBar({ terminalID, open, onOpenChange }: Props) {
  ...
  return <div role="search" aria-label={t('搜索终端内容')} className="absolute right-2 top-2 z-30 flex max-w-[calc(100%-1rem)] items-center gap-1 rounded-xl border border-border bg-card/95 p-1.5 shadow-lg backdrop-blur">
    ...
    <div className="min-w-32 max-w-72 flex-1">
      <Input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={onKeyDown}
        aria-label={t('搜索活跃终端')} aria-invalid={Boolean(error)} placeholder={t('搜索终端内容')} className="h-7 border-0 bg-transparent px-1.5 text-xs shadow-none focus-visible:ring-0" />
      {error ? <p role="alert" className="px-1.5 pt-1 text-[10px] text-destructive">{t('正则表达式无效')}</p> : null}
    </div>
    <span aria-label={t('搜索结果位置')} className="min-w-12 text-center text-[11px] tabular-nums text-muted-foreground">{position}</span>
    <Button type="button" size="icon-xs" variant={regex ? 'secondary' : 'ghost'} aria-label={t('使用正则表达式')} aria-pressed={regex} onClick={() => setRegex((value) => !value)}><Regex /></Button>
    <Button type="button" size="icon-xs" variant="ghost" aria-label={t('上一个搜索结果')} disabled={!query || Boolean(error)} onClick={() => navigate('previous')}><ChevronUp /></Button>
    <Button type="button" size="icon-xs" variant="ghost" aria-label={t('下一个搜索结果')} disabled={!query || Boolean(error)} onClick={() => navigate('next')}><ChevronDown /></Button>
    <Button type="button" size="icon-xs" variant="ghost" aria-label={t('关闭终端搜索')} onClick={close}><X /></Button>
  </div>
}
