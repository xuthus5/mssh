  ...
  return (
    <aside style={panel.panelStyle} className="absolute inset-y-0 right-0 z-20 flex flex-col border-l border-border bg-card shadow-xl" data-testid="command-history-panel">
      ...
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        <span className="flex items-center gap-2 text-sm font-medium"><History className="size-4" />命令历史</span>
        <Button size="icon-xs" variant="ghost" aria-label="关闭历史" onClick={onClose}><X /></Button>
      </div>
      <div className="flex gap-2 border-b border-border p-2">
        <Input placeholder="搜索历史命令..." value={query} onChange={(event) => setQuery(event.target.value)} />
        <Button size="xs" variant="ghost" onClick={clear} disabled={entries.length === 0} title="清空历史"><Trash2 /></Button>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto p-2" onScroll={onScroll}>
        {filtered.length === 0 ? (
          <p className="p-3 text-xs text-muted-foreground">暂无命令历史</p>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1 flex justify-end gap-1 opacity-100 focus-within:opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100">
                    <Button size="xs" variant="ghost" onClick={() => { void copy(entry.command) }}><Clipboard />复制</Button>
                    <Button size="xs" variant="ghost" onClick={() => onFill(entry.command)}>填入终端</Button>
                  </div>
                </div>
              )
            })}
