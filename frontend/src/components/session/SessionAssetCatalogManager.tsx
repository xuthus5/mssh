...
export function SessionAssetCatalogManager(props: Props) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-3">
    <div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-semibold text-foreground">分类管理</h2><p className="text-xs text-muted-foreground">统一维护环境、项目与标签目录。</p></div><Button type="button" onClick={() => setEditor({ kind: tab })}><Plus data-icon="inline-start" />新建{kindLabel(tab)}</Button></div>
    <Tabs value={tab} onValueChange={(value) => setTab(value as CatalogKind)} className="min-h-0 flex-1"><TabsList variant="line"><TabsTrigger value="environment">环境 <Badge variant="secondary">{props.environments.length}</Badge></TabsTrigger><TabsTrigger value="project">项目 <Badge variant="secondary">{props.projects.length}</Badge></TabsTrigger><TabsTrigger value="tag">标签 <Badge variant="secondary">{props.tags.length}</Badge></TabsTrigger></TabsList>
...
function CatalogTable({ kind, items, onEdit, onDelete, onMove }: { kind: CatalogKind; items: (AssetEnvironment | AssetProject | AssetTag)[]; onEdit: (item: AssetEnvironment | AssetProject | AssetTag) => void; onDelete: (item: AssetEnvironment | AssetProject | AssetTag) => void; onMove?: (index: number, direction: -1 | 1) => Promise<void> }) {
  if (items.length === 0) return <Empty className="min-h-64 border"><EmptyHeader><EmptyTitle>暂无{kindLabel(kind)}</EmptyTitle><EmptyDescription>使用右上角按钮创建第一个{kindLabel(kind)}。</EmptyDescription></EmptyHeader></Empty>
  const move = async (index: number, direction: -1 | 1) => {
    if (!onMove) return
    try { await onMove(index, direction) } catch (error) { toast(error instanceof Error ? error.message : String(error), 'error') }
  }
  return <div className="overflow-auto rounded-xl border border-border shadow-sm"><Table><TableHeader><TableRow><TableHead>名称</TableHead>{kind === 'environment' && <TableHead>颜色</TableHead>}{kind === 'project' && <><TableHead>代号</TableHead><TableHead>描述</TableHead></>}<TableHead>关联会话</TableHead>{onMove && <TableHead>排序</TableHead>}<TableHead className="w-20 text-right">操作</TableHead></TableRow></TableHeader>
    <TableBody>{items.map((item, index) => <TableRow key={item.id}><TableCell className="font-medium">{item.name}</TableCell>{kind === 'environment' && <TableCell><Badge variant="outline" data-asset-color={(item as AssetEnvironment).colorToken} className="asset-color-badge">{(item as AssetEnvironment).colorToken}</Badge></TableCell>}{kind === 'project' && <><TableCell>{(item as AssetProject).code || '—'}</TableCell><TableCell className="max-w-64 truncate">{(item as AssetProject).description || '—'}</TableCell></>}<TableCell>{item.sessionCount}</TableCell>{onMove && <TableCell><div className="flex gap-1"><Button type="button" size="icon-xs" variant="ghost" aria-label={`上移 ${item.name}`} disabled={index === 0} onClick={() => { void move(index, -1) }}><ArrowUp /></Button><Button type="button" size="icon-xs" variant="ghost" aria-label={`下移 ${item.name}`} disabled={index === items.length - 1} onClick={() => { void move(index, 1) }}><ArrowDown /></Button></div></TableCell>}<TableCell><DropdownMenu><DropdownMenuTrigger render={<Button type="button" size="icon-xs" variant="ghost" aria-label={`${item.name} 分类操作`} />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuGroup><DropdownMenuItem onClick={() => onEdit(item)}>编辑</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => onDelete(item)}>删除</DropdownMenuItem></DropdownMenuGroup></DropdownMenuContent></DropdownMenu></TableCell></TableRow>)}</TableBody>
  </Table></div>
}
...
function kindLabel(kind: CatalogKind) { return kind === 'environment' ? '环境' : kind === 'project' ? '项目' : '标签' }
