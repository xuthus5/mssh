...
export function SessionAssetBulkBar(props: Props) {
  ...
  return <>
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-muted/40 p-2 shadow-sm">
      <Badge>{props.selectedIDs.length} 个会话</Badge><span className="text-xs text-muted-foreground">批量设置资产归属</span>
      <div className="ml-auto flex items-center gap-1"><Button type="button" size="sm" variant="outline" onClick={() => setKind('environment')}><FolderCog data-icon="inline-start" />环境</Button><Button type="button" size="sm" variant="outline" onClick={() => setKind('project')}>项目</Button><Button type="button" size="sm" variant="outline" onClick={() => setKind('tags')}><Tags data-icon="inline-start" />标签</Button><Button type="button" size="icon-sm" variant="ghost" aria-label="取消批量选择" onClick={props.onClearSelection}><X /></Button></div>
    </div>
...
function BulkAssetDialog({ kind, selectedIDs, environments, projects, tags, onSetEnvironment, onSetProject, onUpdateTags, onClearSelection, onOpenChange }: Props & { kind: BulkKind | null; onOpenChange: (open: boolean) => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast(`已更新 ${count} 个会话的资产信息`, 'success')
      onClearSelection(); onOpenChange(false); setTargetID(''); setTagIDs([]); setOperation('add')
    } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) }
    finally { setPending(false) }
  }
  const title = kind === 'environment' ? '批量设置环境' : kind === 'project' ? '批量设置项目' : '批量更新标签'
  const options = kind === 'environment'
    ? [{ value: '', label: '清空环境' }, ...environments.map((item) => ({ value: item.id, label: item.name }))]
    : [{ value: '', label: '清空项目' }, ...projects.map((item) => ({ value: item.id, label: item.code ? `${item.code} · ${item.name}` : item.name }))]
  return <Dialog open={kind !== null} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>本次操作将影响 {selectedIDs.length} 个会话，并在单个事务中提交。</DialogDescription></DialogHeader>
    ...
    <DialogFooter><Button type="button" variant="outline" disabled={pending} onClick={() => onOpenChange(false)}>取消</Button><Button type="button" disabled={pending || (kind === 'tags' && operation !== 'replace' && tagIDs.length === 0)} onClick={() => { void submit() }}>{pending ? '处理中…' : `确认更新 ${selectedIDs.length} 个会话`}</Button></DialogFooter>
...
function TagBulkFields({ tags, operation, tagIDs, onOperationChange, onTagIDsChange }: { tags: AssetTag[]; operation: TagOperation; tagIDs: string[]; onOperationChange: (operation: TagOperation) => void; onTagIDsChange: (ids: string[]) => void }) {
  return <div className="flex flex-col gap-3">
    <LabeledSelect ariaLabel="标签批量操作" value={operation} options={[{ value: 'add', label: '添加标签' }, { value: 'remove', label: '移除标签' }, { value: 'replace', label: '完全替换标签' }]} onValueChange={(value) => onOperationChange(value as TagOperation)} />
    <DropdownMenu><DropdownMenuTrigger render={<Button type="button" variant="outline" className="justify-start font-normal" />}><Tags data-icon="inline-start" />{tagIDs.length > 0 ? `已选择 ${tagIDs.length} 个标签` : operation === 'replace' ? '不选标签将清空全部标签' : '选择标签'}</DropdownMenuTrigger>
      <DropdownMenuContent className="min-w-64"><DropdownMenuGroup>{tags.length === 0 ? <div className="p-3 text-xs text-muted-foreground">暂无标签</div> : tags.map((tag) => <DropdownMenuCheckboxItem key={tag.id} checked={tagIDs.includes(tag.id)} onCheckedChange={(checked) => onTagIDsChange(checked ? [...tagIDs, tag.id] : tagIDs.filter((id) => id !== tag.id))}>{tag.name}</DropdownMenuCheckboxItem>)}</DropdownMenuGroup></DropdownMenuContent>
    </DropdownMenu>
