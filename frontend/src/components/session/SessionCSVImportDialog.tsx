...
export function SessionCSVImportDialog(props: SessionCSVImportDialogProps) {
  ...
  return <Dialog open={props.open} onOpenChange={state.changeOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl" aria-busy={state.pending}><DialogHeader><DialogTitle>{t('导入会话 CSV')}</DialogTitle><DialogDescription>{t('支持 MSSH 以及常见 PuTTY、SecureCRT、MobaXterm CSV 字段。厂商原生配置文件请先导出为 CSV。')}</DialogDescription></DialogHeader>
    {state.summary ? <SessionCSVImportSummaryView summary={state.summary} /> : <div className="flex flex-col gap-4"><div className="flex flex-wrap items-center gap-2"><Button type="button" variant="outline" disabled={state.pending} onClick={() => { void state.selectFile() }}><FileUp data-icon="inline-start" />{state.preview ? t('重新选择 CSV') : t('选择 CSV 文件')}</Button>{state.path && <span className="max-w-full truncate text-xs text-muted-foreground">{state.path}</span>}</div>{state.error && <p role="alert" className="text-sm text-destructive">{state.error}</p>}
      {state.preview && <><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="mb-2 text-xs font-medium text-muted-foreground">{t('来源模板')}</div><div className="grid gap-2 sm:grid-cols-5">{SESSION_CSV_TEMPLATES.map((template) => <Button key={template.id} type="button" disabled={state.pending} variant={state.provider === template.id ? 'secondary' : 'outline'} className="h-auto min-h-14 justify-start text-left" onClick={() => state.applyProvider(template.id)}><span><span className="block text-sm font-medium">{t(template.label)}</span><span className="mt-0.5 block text-[11px] font-normal text-muted-foreground">{t(template.description)}</span></span></Button>)}</div></div>
        <PreviewSummary preview={state.preview} />
        <SessionCSVMappingTable preview={state.preview} mapping={state.mapping} defaults={state.defaults} disabled={state.pending} onMappingChange={state.setMapping} onDefaultChange={(key, value) => state.setDefaults((current) => ({ ...current, [key]: value }))} />
        {state.requiredMissing.length > 0 && <Alert variant="destructive"><TriangleAlert /><AlertDescription>{t('还需要映射或填写：')}{state.requiredMissing.map((field) => t(field.label)).join('、')}</AlertDescription></Alert>}
        <div className="grid gap-3 sm:grid-cols-2"><LabeledSelect ariaLabel={t('重复会话处理')} value={state.policy} disabled={state.pending} options={[{ value: SessionCSVConflictPolicy.SessionCSVConflictSkip, label: t('跳过重复项') }, { value: SessionCSVConflictPolicy.SessionCSVConflictOverwrite, label: t('覆盖重复项') }]} onValueChange={(value) => state.setPolicy(value as SessionCSVConflictPolicy)} /><p className="self-center text-xs leading-5 text-muted-foreground">{t('重复项按名称、主机、端口和用户名识别。未映射或空单元格使用默认值。')}</p></div>
      </>}</div>}
