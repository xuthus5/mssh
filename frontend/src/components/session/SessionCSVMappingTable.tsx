...
function MappingRow({ field, preview, mapping, defaults, disabled, onMappingChange, onDefaultChange }: SessionCSVMappingTableProps & { field: typeof SESSION_CSV_FIELDS[number] }) {
  const used = new Set(Object.entries(mapping).filter(([key, value]) => key !== field.key && value).map(([, value]) => value))
  const options = [{ value: '', label: t('不映射') }, ...preview.headers.filter((header) => !used.has(header)).map((header) => ({ value: header, label: header }))]
  const source = mapping[field.key] ?? ''
  const fieldLabel = t(field.label)
  return <tr className="border-b border-border last:border-0"><td className="px-3 py-2 align-top"><span className="font-medium">{fieldLabel}</span>{field.required && <span className="ml-1 text-destructive">*</span>}<div className="text-[11px] text-muted-foreground">{field.key}</div></td><td className="px-3 py-2 align-top"><LabeledSelect ariaLabel={t('${}外部表头', fieldLabel)} value={source} options={options} disabled={disabled} onValueChange={(value) => onMappingChange(updateSessionCSVMapping(mapping, field.key, value))} placeholder={t('选择外部表头')} /></td><td className="px-3 py-2 align-top"><Input aria-label={t('${}默认值', fieldLabel)} value={defaults[field.key] ?? ''} disabled={disabled} placeholder={field.placeholder} onChange={(event) => onDefaultChange(field.key, event.target.value)} /></td><td className="max-w-72 px-3 py-2 align-top text-xs text-muted-foreground"><span className="break-words">{source ? sessionCSVSample(preview.headers, preview.sample_rows, source) || t('（空）') : t('未映射')}</span></td></tr>
}
