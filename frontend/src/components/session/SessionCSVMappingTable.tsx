...
export function SessionCSVMappingTable(props: SessionCSVMappingTableProps) {
  return <div className="overflow-hidden rounded-xl border border-border"><div className="border-b border-border bg-muted/40 px-3 py-2"><div className="text-sm font-medium">字段映射</div><div className="mt-0.5 text-xs text-muted-foreground">一个外部表头只能对应一个 MSSH 字段；未映射字段使用默认值。</div></div><div className="max-h-[38vh] overflow-auto"><table className="w-full min-w-[760px] text-sm"><thead className="sticky top-0 z-10 bg-background"><tr className="border-b border-border"><th className="w-36 px-3 py-2 text-left font-medium">MSSH 字段</th><th className="w-64 px-3 py-2 text-left font-medium">外部表头</th><th className="w-52 px-3 py-2 text-left font-medium">默认值</th><th className="px-3 py-2 text-left font-medium">样例</th></tr></thead><tbody>{SESSION_CSV_FIELDS.map((field) => <MappingRow key={field.key} field={field} {...props} />)}</tbody></table></div></div>
}
...
function MappingRow({ field, preview, mapping, defaults, onMappingChange, onDefaultChange }: SessionCSVMappingTableProps & { field: typeof SESSION_CSV_FIELDS[number] }) {
  const options = useMemo(() => {
    ...
    return [{ value: '', label: '不映射' }, ...preview.headers.filter((header) => !used.has(header)).map((header) => ({ value: header, label: header }))]
  }, [field.key, mapping, preview.headers])
  const source = mapping[field.key] ?? ''
  return <tr className="border-b border-border last:border-0"><td className="px-3 py-2 align-top"><span className="font-medium">{field.label}</span>{field.required && <span className="ml-1 text-destructive">*</span>}<div className="text-[11px] text-muted-foreground">{field.key}</div></td><td className="px-3 py-2 align-top"><LabeledSelect ariaLabel={`${field.label}外部表头`} value={source} options={options} onValueChange={(value) => onMappingChange(updateSessionCSVMapping(mapping, field.key, value))} placeholder="选择外部表头" /></td><td className="px-3 py-2 align-top"><Input aria-label={`${field.label}默认值`} value={defaults[field.key] ?? ''} placeholder={field.placeholder} onChange={(event) => onDefaultChange(field.key, event.target.value)} /></td><td className="max-w-72 px-3 py-2 align-top text-xs text-muted-foreground"><span className="break-words">{source ? sessionCSVSample(preview.headers, preview.sample_rows, source) || '（空）' : '未映射'}</span></td></tr>
}
