...
export function SessionAssetCenter() {
  ...
  return <section className="flex min-h-0 flex-1 flex-col bg-background p-5">
    ...
    <div className="mt-4"><SessionBatchActions selectedIDs={[...selectedIDs]} onBatchConnect={state.batchConnect} onBatchExecuteMacro={state.batchExecuteMacro} onComplete={() => setSelectedIDs(new Set())} /></div>
    <Tabs value={tab} onValueChange={(value) => setTab(value as AssetTab)} className="mt-4 min-h-0 flex-1">
      ...
      <TabsContent value="recent" className="min-h-0 overflow-auto pt-4">{state.loading ? <LoadingRows /> : <SessionTable sessions={state.recentSessions} folders={state.folders} selectedIDs={selectedIDs} onSelectionChange={setSelectedIDs} onConnect={state.connect} onEdit={editSession} onDelete={(session) => setDeleteTarget({ type: 'session', item: session })} onMove={state.moveSession} recent />}</TabsContent>
...
function SessionTable({ sessions, folders, selectedIDs, onSelectionChange, onConnect, onEdit, onDelete, onMove, recent = false }: { sessions: Session[]; folders: Folder[]; selectedIDs: Set<string>; onSelectionChange: (ids: Set<string>) => void; onConnect: (id: string) => void; onEdit: (session: Session) => void; onDelete: (session: Session) => void; onMove: (id: string, folderID: string | null) => void; recent?: boolean }) {
  ...
  const allSelected = sessions.every((session) => selectedIDs.has(session.id))
  const toggleAll = () => { const next = new Set(selectedIDs); sessions.forEach((session) => allSelected ? next.delete(session.id) : next.add(session.id)); onSelectionChange(next) }
  const toggleOne = (id: string) => { const next = new Set(selectedIDs); if (next.has(id)) next.delete(id); else next.add(id); onSelectionChange(next) }
  return <div className="rounded-lg border"><Table><TableHeader><TableRow><TableHead className="w-10"><Checkbox aria-label="选择当前列表全部会话" checked={allSelected} onCheckedChange={toggleAll} /></TableHead><TableHead>名称</TableHead><TableHead>端点</TableHead><TableHead>分组</TableHead>{recent && <TableHead>最近连接</TableHead>}<TableHead className="w-24 text-right">操作</TableHead></TableRow></TableHeader><TableBody>{sessions.map((session) => <TableRow key={session.id} data-state={selectedIDs.has(session.id) ? 'selected' : undefined}><TableCell><Checkbox aria-label={`选择 ${session.name}`} checked={selectedIDs.has(session.id)} onCheckedChange={() => toggleOne(session.id)} /></TableCell><TableCell className="font-medium">{session.name}</TableCell><TableCell>{session.username}@{session.host}:{session.port}</TableCell><TableCell>{folders.find((folder) => folder.id === session.folderId)?.name ?? '未分组'}</TableCell>{recent && <TableCell>{formatRecent(session)}</TableCell>}<TableCell><div className="flex justify-end gap-1"><Button size="xs" onClick={() => onConnect(session.id)}>连接</Button><DropdownMenu><DropdownMenuTrigger render={<Button size="icon-xs" variant="ghost" aria-label={`${session.name} 更多操作`} />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuGroup><DropdownMenuItem onClick={() => onEdit(session)}>编辑</DropdownMenuItem><DropdownMenuSub><DropdownMenuSubTrigger>移动到分组</DropdownMenuSubTrigger><DropdownMenuSubContent><DropdownMenuGroup>{folders.map((folder) => <DropdownMenuItem key={folder.id} disabled={folder.id === session.folderId} onClick={() => onMove(session.id, folder.id)}>{folder.name}</DropdownMenuItem>)}</DropdownMenuGroup></DropdownMenuSubContent></DropdownMenuSub><DropdownMenuItem variant="destructive" onClick={() => onDelete(session)}>删除</DropdownMenuItem></DropdownMenuGroup></DropdownMenuContent></DropdownMenu></div></TableCell></TableRow>)}</TableBody></Table></div>
}
