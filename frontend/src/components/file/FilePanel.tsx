...
function MkdirForm({ name, onChange, onSubmit }: { name: string; onChange: (name: string) => void; onSubmit: (event: FormEvent) => void }) {
  return <form onSubmit={onSubmit} className="flex items-center gap-1 border-b border-border px-3 py-1.5"><input className="h-7 flex-1 rounded border border-input bg-transparent px-2 text-sm outline-none" placeholder="文件夹名" value={name} onChange={(event) => onChange(event.target.value)} autoFocus /><Button size="xs" type="submit">确定</Button></form>
}
...
function PanelHeader({ onClose, onSyncCurrentDirectory, syncingCurrentDirectory }: { onClose: () => void; onSyncCurrentDirectory: () => void; syncingCurrentDirectory: boolean }) {
  return <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2"><span className="text-sm font-medium">文件管理</span><div className="flex shrink-0 items-center gap-1"><Button size="xs" variant="outline" title="同步当前终端的 Shell 工作目录（需要处于 Shell 提示符）" disabled={syncingCurrentDirectory} onClick={onSyncCurrentDirectory}><RefreshCw className={syncingCurrentDirectory ? 'animate-spin' : undefined} data-icon="inline-start" />{syncingCurrentDirectory ? '同步中' : '同步当前目录'}</Button><Button size="xs" variant="ghost" onClick={onClose}>关闭</Button></div></div>
}
...
function FileDialogs({ selected, renameOpen, renameName, deleteOpen, onRenameOpenChange, onRenameNameChange, onDeleteOpenChange, onRename, onDelete, onClearSelection }: { selected: FileInfo | null; renameOpen: boolean; renameName: string; deleteOpen: boolean; onRenameOpenChange: (open: boolean) => void; onRenameNameChange: (name: string) => void; onDeleteOpenChange: (open: boolean) => void; onRename: (path: string, name: string) => void; onDelete: (path: string) => void; onClearSelection: () => void }) {
  return <><Dialog open={renameOpen} onOpenChange={onRenameOpenChange}><DialogContent><DialogHeader><DialogTitle>重命名</DialogTitle></DialogHeader><Input value={renameName} onChange={(event) => onRenameNameChange(event.target.value)} autoFocus /><DialogFooter showCloseButton><Button onClick={() => { if (selected && renameName.trim()) onRename(selected.path, renameName.trim()); onRenameOpenChange(false) }}>保存</Button></DialogFooter></DialogContent></Dialog><AlertDialog open={deleteOpen} onOpenChange={onDeleteOpenChange}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>删除“{selected?.name}”</AlertDialogTitle><AlertDialogDescription>远程文件删除后无法恢复。</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>取消</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => { if (selected) onDelete(selected.path); onClearSelection() }}>删除</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></>
}
