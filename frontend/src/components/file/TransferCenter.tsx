...
export function TransferCenter() {
  ...
  const reloadTransfers = () => {
    void restoreTransfers().catch((error: unknown) => {
      const message = error instanceof Error ? error.message : String(error)
      setTransfersLoadError(message)
    })
  }
  ...
  return <>
    {(transfers.length > 0 || loadError) && <Button type="button" size="xs" variant="ghost" aria-label={triggerLabel} onClick={() => setOpen(true)}>
      <ArrowUpDown data-icon="inline-start" />
      {t('传输')}
      <Badge variant={loadError ? 'destructive' : 'secondary'}>{loadError ? '!' : (summary.activeCount > 0 ? `${summary.activeCount}${summary.percentage === null ? '' : ` · ${summary.percentage}%`}` : recent.length)}</Badge>
    </Button>}
