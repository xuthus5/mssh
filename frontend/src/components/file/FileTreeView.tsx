...
export function FileTreeView(props: Props) {
  ...
  return (
    <div role="tree" aria-label="远程文件树" aria-activedescendant={active ? `file-${active.file.path}` : undefined} tabIndex={0} className="min-w-0 py-1 outline-none" onKeyDown={onKeyDown}>
      {props.loading ? <TreeEmpty text="加载中..." /> : flat.length === 0 ? <TreeEmpty text="空目录" /> : flat.map((node, index) => {
