...
export function AIAgentTaskWorkspace({ controller }: { controller: ReturnType<typeof useAIAgentTasks> }) {
  ...
  return <div className="grid min-h-0 flex-1 md:grid-cols-[minmax(180px,0.36fr)_minmax(0,1fr)]">
    ...
    <div className="min-h-0 overflow-y-auto"><TaskDetail task={controller.selected} pending={controller.pending} approve={controller.approve} cancel={controller.cancel} resume={controller.resume} remove={controller.remove} /></div>
  </div>
}
...
function TaskDetail({ task, pending, approve, cancel, resume, remove }: { task: AIAgentTask | null; pending: string | null; approve: ReturnType<typeof useAIAgentTasks>['approve']; cancel: ReturnType<typeof useAIAgentTasks>['cancel']; resume: ReturnType<typeof useAIAgentTasks>['resume']; remove: ReturnType<typeof useAIAgentTasks>['remove'] }) {
  if (!task) return null
