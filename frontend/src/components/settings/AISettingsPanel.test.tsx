...
describe('AISettingsPanel', () => {
  ...
  it('saves network search configuration', async () => {
    ...
    expect(screen.getByRole('combobox', { name: '搜索模式' }).textContent).toContain('独立搜索')
    await selectOption(user, '搜索提供商', 'Tavily')
    expect(screen.getByRole('combobox', { name: '搜索提供商' }).textContent).toContain('Tavily')
    expect(screen.getByRole('combobox', { name: '搜索提供商' }).textContent).not.toContain('tavily')
