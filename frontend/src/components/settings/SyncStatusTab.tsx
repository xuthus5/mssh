...
export function SyncStatusTab(props: Props) {
  ...
  return <div className="flex flex-col gap-5">
    ...
    {props.dirty && <Alert><AlertTitle>存在未保存配置</AlertTitle><AlertDescription>保存 Provider、密钥或同步策略后才能执行同步。</AlertDescription></Alert>}
    {props.error && <Alert variant="destructive"><AlertDescription>{props.error}</AlertDescription></Alert>}
    <div className="flex flex-wrap gap-2"><Button type="button" disabled={actionsDisabled} onClick={() => void props.onSync().catch(() => undefined)}><RefreshCw data-icon="inline-start" />立即同步</Button><Button type="button" variant="outline" disabled={actionsDisabled} onClick={() => void props.onPush().catch(() => undefined)}><ArrowUpFromLine data-icon="inline-start" />推送本地</Button><Button type="button" variant="outline" disabled={actionsDisabled} onClick={() => void props.onPull().catch(() => undefined)}><ArrowDownToLine data-icon="inline-start" />拉取云端</Button></div>
...
function SyncOverview({ dashboard }: { dashboard: SyncDashboard }) {
  ...
  return <section><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2"><h3 className="text-sm font-semibold">{syncProviderLabel(dashboard.config.provider)}</h3><Badge variant={badgeVariant}>{syncStateLabel(dashboard.state)}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{dashboard.message || '等待同步操作'} · 最近同步 {formatSyncDate(dashboard.last_synced_at)}</p></div></div>
    <div className="mt-4 grid overflow-hidden rounded-lg border border-border bg-muted/20 md:grid-cols-2 md:divide-x md:divide-border"><VersionSummary title="本地版本" version={dashboard.local_version?.version_number} fingerprint={dashboard.local_version?.snapshot_fingerprint} time={dashboard.local_version?.created_at} /><VersionSummary title="云端版本" version={dashboard.remote_version?.version_number} fingerprint={dashboard.remote_version?.snapshot_fingerprint} time={dashboard.remote_version?.created_at} /></div>
  </section>
}

function VersionSummary(props: { title: string; version?: number; fingerprint?: string; time?: string }) {
  return <div className="px-3 py-3"><div className="text-xs font-medium text-muted-foreground">{props.title}</div><div className="mt-1 text-lg font-semibold">{props.version ? `v${props.version}` : '未建立'}</div><div className="mt-1 truncate font-mono text-[11px] text-muted-foreground">{props.fingerprint ? props.fingerprint.slice(0, 16) : '—'} · {formatSyncDate(props.time)}</div></div>
}
...
function ConflictPanel({ pending, onResolve }: { pending: boolean; onResolve: (choice: SyncConflictChoice) => Promise<void> }) {
  ...
  return <Alert variant="destructive"><AlertTriangle /><AlertTitle>本地与云端都发生了变化</AlertTitle><AlertDescription><p className="mb-3">请选择本次冲突的处理方式；取消会保留当前状态。</p><div className="flex flex-wrap gap-2"><Button type="button" size="sm" disabled={pending} onClick={() => resolve(SyncConflictChoice.SyncConflictUseLocal)}>采用本地</Button><Button type="button" size="sm" variant="outline" disabled={pending} onClick={() => resolve(SyncConflictChoice.SyncConflictUseCloud)}>采用云端</Button><Button type="button" size="sm" variant="ghost" disabled={pending} onClick={() => resolve(SyncConflictChoice.SyncConflictCancel)}>暂不处理</Button></div></AlertDescription></Alert>
}
