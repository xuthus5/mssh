...
function ColorField({ id, label, value, onChange }: ColorFieldProps) {
  ...
  return <Field data-invalid={!valid}>
    ...
    <div className="grid grid-cols-[3rem_minmax(0,1fr)] gap-2">
      <input aria-label={t('${}选择器', label)} type="color" value={safeHexColor(value)} onChange={(event) => onChange(event.target.value)} className="size-8 self-center rounded-lg border border-input bg-transparent p-0.5" />
...
export function TerminalThemeInspector({ theme, onThemeChange }: Props) {
  return <Card>
    <CardHeader><CardTitle className="flex items-center gap-2 text-sm"><Palette className="size-4" />{t('基础颜色')}</CardTitle></CardHeader>
