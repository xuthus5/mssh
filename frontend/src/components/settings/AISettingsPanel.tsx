...
export function AISettingsPanel({ controller }: { controller: AISettingsController }) {
  ...
  if (controller.loading && !dashboard) return <p className="p-8 text-center text-sm text-muted-foreground">正在加载 AI 配置...</p>
  if (!dashboard || !draft) return <p className="p-8 text-center text-sm text-destructive">AI 配置加载失败</p>
...
function InteractionPanel({ draft, update, onSave, pending }: { draft: AISettingsInput; update: (changes: Partial<AISettingsInput>) => void; onSave: () => Promise<void>; pending: boolean }) {
  ...
  return <SettingsCard icon={<SlidersHorizontal />} title="交互配置" description="控制右侧 AI 面板携带的上下文和对话保留方式。"><div className="grid gap-4 sm:grid-cols-2"><NumberField label="面板宽度" value={interaction.panel_width} min={300} max={900} onChange={(value) => setInteraction({ panel_width: value })} /><NumberField label="终端上下文行数" value={interaction.context_lines} min={0} max={500} onChange={(value) => setInteraction({ context_lines: value })} /><NumberField label="历史保留天数" value={interaction.history_retention_days} min={1} max={3650} onChange={(value) => setInteraction({ history_retention_days: value })} /><NumberField label="最多对话数" value={interaction.max_conversations} min={1} max={1000} onChange={(value) => setInteraction({ max_conversations: value })} /></div><div className="grid gap-3 sm:grid-cols-2"><SettingSwitch label="附带会话信息" checked={interaction.include_session_metadata} onChange={(value) => setInteraction({ include_session_metadata: value })} /><SettingSwitch label="附带系统摘要" checked={interaction.include_system_summary} onChange={(value) => setInteraction({ include_system_summary: value })} /><SettingSwitch label="流式响应" checked={interaction.stream_responses} onChange={(value) => setInteraction({ stream_responses: value })} /><SettingSwitch label="自动滚动" checked={interaction.auto_scroll} onChange={(value) => setInteraction({ auto_scroll: value })} /><SettingSwitch label="渲染 Markdown" checked={interaction.render_markdown} onChange={(value) => setInteraction({ render_markdown: value })} /></div><SaveButton pending={pending} onSave={onSave} /></SettingsCard>
}
...
function SearchPanel({ draft, update, onSave, pending, saved }: { draft: AISettingsInput; update: (changes: Partial<AISettingsInput>) => void; onSave: () => Promise<void>; pending: boolean; saved: boolean }) {
  ...
  return <SettingsCard icon={<Globe2 />} title="网络搜索" description="将搜索结果作为模型上下文，并在回答中保留引用。"><SettingSwitch label="启用网络搜索" checked={search.enabled} onChange={(value) => setSearch({ enabled: value })} /><div className="grid gap-4 sm:grid-cols-2"><Field label="搜索模式"><Select value={search.mode} onValueChange={(value) => setSearch({ mode: value as typeof search.mode })}><SelectTrigger aria-label="搜索模式" className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="auto">自动判断</SelectItem><SelectItem value="native">提供商原生</SelectItem><SelectItem value="independent">独立搜索</SelectItem><SelectItem value="disabled">禁用</SelectItem></SelectContent></Select></Field><Field label="搜索提供商"><Select value={search.provider} onValueChange={(value) => setSearch({ provider: value as typeof search.provider })}><SelectTrigger aria-label="搜索提供商" className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="brave">Brave</SelectItem><SelectItem value="tavily">Tavily</SelectItem><SelectItem value="serper">Serper</SelectItem></SelectContent></Select></Field><NumberField label="超时（秒）" value={search.timeout_seconds} min={1} max={60} onChange={(value) => setSearch({ timeout_seconds: value })} /><NumberField label="最大结果数" value={search.max_results} min={1} max={20} onChange={(value) => setSearch({ max_results: value })} /></div><Field label={saved ? '搜索 API Key（已保存，留空保持不变）' : '搜索 API Key'}><Input aria-label="搜索 API Key" type="password" value={search.api_key} onChange={(event) => setSearch({ api_key: event.target.value })} autoComplete="new-password" /></Field><SettingSwitch label="要求回答提供引用" checked={search.require_citations} onChange={(value) => setSearch({ require_citations: value })} /><SaveButton pending={pending} onSave={onSave} /></SettingsCard>
}
...
function SecurityPanel({ draft, update, onSave, pending }: { draft: AISettingsInput; update: (changes: Partial<AISettingsInput>) => void; onSave: () => Promise<void>; pending: boolean }) {
  ...
  return <SettingsCard icon={<ShieldCheck />} title="安全配置" description="命令默认需要审批；内置高风险阻断规则始终生效。"><SettingSwitch label="允许只读命令自动执行" checked={security.auto_execute_read_only} onChange={(value) => setSecurity({ auto_execute_read_only: value })} /><div className="grid gap-4 sm:grid-cols-3"><NumberField label="命令超时（秒）" value={security.command_timeout_seconds} min={1} max={300} onChange={(value) => setSecurity({ command_timeout_seconds: value })} /><NumberField label="最大输出字节" value={security.max_output_bytes} min={1024} max={4194304} onChange={(value) => setSecurity({ max_output_bytes: value })} /><NumberField label="计划最多步骤" value={security.max_plan_steps} min={1} max={20} onChange={(value) => setSecurity({ max_plan_steps: value })} /></div><div className="grid gap-4 sm:grid-cols-3"><PatternField label="允许模式" value={security.allow_patterns} onChange={(value) => setSecurity({ allow_patterns: value })} /><PatternField label="禁止模式" value={security.deny_patterns} onChange={(value) => setSecurity({ deny_patterns: value })} /><PatternField label="脱敏模式" value={security.redaction_patterns} onChange={(value) => setSecurity({ redaction_patterns: value })} /></div><SaveButton pending={pending} onSave={onSave} /></SettingsCard>
}
...
function PatternField({ label, value, onChange }: { label: string; value: string[]; onChange: (value: string[]) => void }) { return <Field label={`${label}（每行一个正则）`}><Textarea aria-label={label} value={value.join('\n')} onChange={(event) => onChange(event.target.value.split('\n').map((item) => item.trim()).filter(Boolean))} rows={4} /></Field> }
...
function SaveButton({ pending, onSave }: { pending: boolean; onSave: () => Promise<void> }) { return <div className="flex justify-end"><Button disabled={pending} onClick={() => void onSave()}><Save data-icon="inline-start" />保存配置</Button></div> }
