...
function OverviewTunnels() {
  ...
  return <section className="flex min-h-0 flex-1 flex-col overflow-auto bg-background p-5"><header className="mb-5 flex items-center gap-3"><Network className="size-5 text-primary" /><div><h1 className="text-xl font-semibold">隧道配置</h1><p className="text-sm text-muted-foreground">按会话管理本地、远程和动态转发</p></div></header><Card><CardHeader className="flex flex-row items-center justify-between gap-4"><CardTitle className="text-sm">隧道列表</CardTitle><div className="flex items-center gap-2"><LabeledSelect ariaLabel="隧道所属会话" value={sessionID} options={workspace.sessions.map((session) => ({ value: session.id, label: session.name }))} onValueChange={setSessionID} /><Button type="button" disabled={!selectedSession} onClick={() => setOpen(true)}>管理隧道</Button></div></CardHeader><CardContent>{selectedSession ? <TunnelTable tunnels={manager.tunnels} /> : <div className="flex items-center gap-3 text-sm text-muted-foreground"><Server className="size-4" />请先创建会话</div>}</CardContent></Card><TunnelDialog open={open} onOpenChange={setOpen} tunnels={manager.tunnels} onStart={manager.start} onStop={manager.stop} sessionId={sessionID} /></section>
}

function TunnelTable({ tunnels }: { tunnels: ReturnType<typeof useTunnelManager>['tunnels'] }) {
  return <div className="rounded-xl border"><Table><TableHeader><TableRow><TableHead>类型</TableHead><TableHead>本地端点</TableHead><TableHead>远程端点</TableHead><TableHead>状态</TableHead></TableRow></TableHeader><TableBody>{tunnels.length === 0 ? <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground">当前会话暂无隧道</TableCell></TableRow> : tunnels.map((tunnel) => <TableRow key={tunnel.id}><TableCell>{tunnelTypeLabel(tunnel.type)}</TableCell><TableCell>{tunnel.localAddress}:{tunnel.localPort}</TableCell><TableCell>{tunnel.type === 'dynamic' ? '-' : `${tunnel.remoteAddress}:${tunnel.remotePort}`}</TableCell><TableCell><Badge variant={tunnel.running ? 'default' : 'outline'}>{tunnel.running ? '运行中' : '已停止'}</Badge></TableCell></TableRow>)}</TableBody></Table></div>
}

function tunnelTypeLabel(type: 'local' | 'remote' | 'dynamic') {
  return ({ local: '本地转发', remote: '远程转发', dynamic: '动态转发' })[type]
}
