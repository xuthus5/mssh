...
function OverviewKeys() {
  ...
  return <section className="flex min-h-0 flex-1 flex-col overflow-auto bg-background p-5"><header className="mb-5 flex items-center gap-3"><KeyRound className="size-5 text-primary" /><div><h1 className="text-xl font-semibold">{t('密钥配置')}</h1><p className="text-sm text-muted-foreground">{t('管理 SSH 身份与公私钥材料')}</p></div><Badge variant="secondary" className="ml-auto">{settings.keys.length} {t('个密钥')}</Badge></header><Card><CardHeader><CardTitle className="text-sm">{t('SSH 密钥')}</CardTitle></CardHeader><CardContent><KeyManager keys={settings.keys} loadError={settings.error} loading={settings.loading} onReload={settings.listKeys} onGenerate={settings.generateKey} onImport={settings.importKey} onDelete={settings.deleteKey} onExport={settings.exportKey} onLoadMaterial={settings.loadKeyMaterial} onUpdate={settings.updateKey} onSelectImportFile={settings.selectKeyImportFile} /></CardContent></Card></section>
}
...
function OverviewTunnels() {
  ...
  return <section className="flex min-h-0 flex-1 flex-col overflow-auto bg-background p-5"><header className="mb-5 flex items-center gap-3"><Network className="size-5 text-primary" /><div><h1 className="text-xl font-semibold">{t('隧道配置')}</h1><p className="text-sm text-muted-foreground">{t('按会话管理本地、远程和动态转发')}</p></div></header><Card><CardHeader className="flex flex-row items-center justify-between gap-4"><CardTitle className="text-sm">{t('隧道列表')}</CardTitle><div className="flex items-center gap-2"><LabeledSelect ariaLabel={t('隧道所属会话')} value={sessionID} options={workspace.sessions.map((session) => ({ value: session.id, label: session.name }))} onValueChange={setSessionID} /><Button type="button" variant="outline" disabled={manager.loading} onClick={() => { void manager.load() }}>{t('刷新')}</Button><Button type="button" disabled={!selectedSession} onClick={() => setOpen(true)}>{t('管理隧道')}</Button></div></CardHeader><CardContent>{manager.error ? <Alert variant="destructive" className="mb-3"><AlertDescription>{t('加载隧道失败: ${}', manager.error)}<Button size="xs" variant="outline" className="ml-2" onClick={() => { void manager.load() }}>{t('重试')}</Button></AlertDescription></Alert> : null}{selectedSession ? <TunnelTable tunnels={manager.tunnels} /> : <div className="flex items-center gap-3 text-sm text-muted-foreground"><Server className="size-4" />{t('请先创建会话')}</div>}</CardContent></Card><TunnelDialog open={open} onOpenChange={setOpen} tunnels={manager.tunnels} loadError={manager.error} onReload={() => manager.load()} onStart={manager.start} onStop={manager.stop} onDelete={manager.remove} sessionId={sessionID} /></section>
}
