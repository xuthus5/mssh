...
function SessionPanel({ workspace, filter, editSession, dialogs }: {
  ...
  const expandedFolderIDs = useAppStore((state) => state.sessionTreeExpandedFolderIDs)
  const setExpandedFolderIDs = useAppStore((state) => state.setSessionTreeExpandedFolderIDs)
  ...
  return <>
    ...
    <div className="min-h-0 flex-1">
      {workspace.error ? <Alert variant="destructive" className="m-2"><AlertDescription>{workspace.error}<Button size="xs" variant="outline" className="mt-2" onClick={retry}>{t('重试')}</Button></AlertDescription></Alert> : !hasSessionData && workspace.loading ? <div className="flex flex-col gap-2 p-3"><Skeleton className="h-7 w-full" /><Skeleton className="h-7 w-4/5" /><Skeleton className="h-7 w-3/5" /></div> : <SessionTree folders={filter.filteredFolders} sessions={filter.filteredSessions} onConnect={workspace.connect} onEditSession={editSession} onDuplicateSession={dialogs.duplicateSession} onQuickRenameSession={dialogs.quickRenameSession} onCopyConnectionInfo={dialogs.copyConnectionInfo} onCopyCredentials={dialogs.copyCredentials} onDeleteSession={(sessionId) => { const session = workspace.sessions.find((item) => item.id === sessionId); if (session) void dialogs.deleteSession(session) }} onEditFolder={dialogs.editFolder} onDeleteFolder={(folderId) => { const folder = workspace.folders.find((item) => item.id === folderId); if (folder) void dialogs.deleteFolder(folder) }} onSelectFolder={(id) => window.dispatchEvent(new CustomEvent('mssh:select-folder', { detail: id }))} expandedFolderIDs={expandedFolderIDs} onExpandedFolderIDsChange={setExpandedFolderIDs} revealAll={Boolean(filter.searchQuery.trim())} />}
    </div>
  </>
}
